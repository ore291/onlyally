...
import { removeCartDetailsStart, saveCartDetailsStart } from "../../store/slices/productsSlice";
  ...
  const handleButtonClick = (action)=>{
    ...
    if(action == 'increase'){
      if(cart.user_product.quantity > count){
        dispatch(saveCartDetailsStart({cart_id: cart.cart_id,user_product_id: cart.user_product_id, quantity: count+1}));
 ...
 return (
        ...
        <tbody>
          ...
          <tr>
            ...
            <td className="p-4 md:px-4 text-center whitespace-nowrap">{cart.user_product.name}</td>
            <td className="p-4 md:px-4 text-center whitespace-nowrap">
            <div className="w-20 flex h-8">
                        <button onClick={(cart)=> handleButtonClick("decrease")} className="w-2/5 border-1 border-stone-650 h-full bg-stone-400">-</button>
                        <input className="w-3/5 border-1 text-lg font-normal border-stone-650 outline-none m-0 p-0  appearance-none"
                         ...
                         value={cart.quantity}                
                         />
                        <button  onClick={(cart)=> handleButtonClick( "increase")}  className="w-2/5 border-1 border-stone-650 h-full bg-stone-400">+</button>
                    </div>
            </td>
