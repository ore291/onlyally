...
import { deleteUserProductStart, fetchUserProductsStart } from "../../store/slices/productsSlice";
...
function ProductList() {
  ...
  useEffect(() => {
   ...
   console.log(products.data.user_products)
              ...
              <p className="flex-1">{(data.is_visible== 1) ? 'Yes' : 'No'}</p>
               <div className="flex  flex-1 flex-col ">
                  <div className="flex  w-[80%] justify-around text-gray-400">
                        <ImCross onClick={() => deleteProduct(data.user_product_id)} className="cursor-pointer  hover:text-gray-500" size="20"/>
                         <Link href={`/single-product/${data.unique_id}`} >
                        <FiEye className="cursor-pointer  hover:text-gray-500"  size="23"/>
                         </Link>
                         <Link href={`/edit-product/${data.unique_id}`} >
                        <FaEdit className="cursor-pointer  hover:text-gray-500s"  size="20"/>
                        </Link>
                   </div>
