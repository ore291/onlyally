...
function* fetchCommentsAPI() {
  try {
    const inputData = yield select(
      (state) => state.comments.comments.inputData
    );
...
function* fetchCommentRepliesAPI() {
  try {
    const inputData = yield select(
      (state) => state.comments.commentReplies.inputData
    );
    const response = yield api.postMethod({
      action: "post_comment_replies",
      object: inputData,
    });
    if (response.data.success) {
...
function* saveCommentReplyAPI() {
  try {
    const inputData = yield select(
      (state) => state.comments.saveCommentReply.inputData
    );
    const response = yield api.postMethod({
      action: "post_comments_replies_save",
      object: inputData,
    });
    if (response.data.success) {
...
export default function* pageSaga() {
  yield all([
    yield takeLatest("comments/fetchCommentsStart", fetchCommentsAPI),
  ]);
  yield all([
    yield takeLatest(
      "comments/fetchCommentRepliesStart",
      fetchCommentRepliesAPI
    ),
  ]);
  yield all([yield takeLatest("comments/saveCommentStart", saveCommentAPI)]);
  yield all([
    yield takeLatest("comments/saveCommentRepliesStart", saveCommentReplyAPI),
  ]);
